...
# Ссылка из Apps Script (заменим, когда будете дома)
...
def load_all_data():
    try:
        ...
        try:
            ...
            df_r = pd.DataFrame(columns=['Вес', 'Загрузка', 'Имя', 'Перед_Сжатие', 'Перед_Отбой', 'Перед_Преднатяг_Витков', 'Зад_Преднатяг', 'Зад_Отбой', 'Зад_Реальный_Сэг_мм', 'Причина_Текст'])
...
if df_base is not None:
    ...
    with st.expander("✍️ Добавить свой вариант настройки / Предложить изменения"):
        ...
        if st.button("🚀 Опубликовать сетап в приложении"):
            if not user_name.strip() or not user_comment.strip():
                ...
                payload = {
                    "weight": rounded_weight, "mode": loading_mode, "name": user_name,
                    "p_szh": int(u_p_szh), "p_otb": int(u_p_otb), "p_seg": float(u_p_tur),
    ...
    # ==================== НОВЫЙ БЛОК: ИЛЛЮСТРИРОВАННОЕ РУКОВОДСТВО ====================
    ...
    with st.expander("🛠️ ИНСТРУКЦИЯ ДЛЯ ПЕРЕДНЕЙ ВИЛКИ"):
        ...
        # Место под ваше фото вилки (заменим ссылку на ваше реальное фото позже)
        ...
        st.info("⚙️ **Где находятся винты:** Регулировка отбоя (TEN) и сжатия (COMP) обычно разделены по перьям (левое/правое) или находятся на торцах сверху. Сжатие отвечает за жесткость при наезде на кочку, отбой — за скорость возврата вилки.")
        
        st.write("🌀 **ПРЕДНАТЯГ ВИЛКИ (Витки):** Регулируется большой гайкой под ключ сверху пера.")
        st.write("• Отсчет идет **от полностью распущенного состояния** (крутить против часовой стрелки до самого верха).")
        st.write("• Чтобы зажать пружину по калькулятору, вращайте гайку по часовой стрелке, отсчитывая полные обороты (витки).")

    with st.expander("🛠️ ИНСТРУКЦИЯ ДЛЯ ЗАДНЕГО АМОРТИЗАТОРА"):
        st.markdown("### 🪐 Настройка заднего амортизатора")
        
        # Место под ваше фото заднего регулятора
      st.image("unsplash.com", caption="Выносная рукоятка преднатяга и нижний винт отбоя", use_container_width=True)st.write("🌀 ПРЕДНАТЯГ ПРУЖИНЫ (Щелчки): Регулируется большой выносной пластиковой рукояткой на правом боку мотоцикла.")st.write("• Отсчет идет от полностью распущенного состояния (крутить против часовой стрелки до упора, когда крутилка станет идти совсем легко).")st.write("• Чтобы настроить под вес, крутите рукоятку по часовой стрелке, считая каждый отчетливый щелчок.")st.write("🛑 ОТБОЙ АМОРТИЗАТОРA: Регулируется маленьким винтом под шлицевую отвертку в самой нижней части амортизатора (около прогрессии).")st.write("• Закрутите винт по часовой стрелке до упора (в сторону H).")st.write("• Открутите против часовой стрелки (в сторону S) на нужное количество кликов.")# Раздел с теорией хода подвескиwith st.expander("📊 Теория сэга и ходов подвески"):st.write("• Ход передней вилки: 194 мм  |  Ход заднего амортизатора: 198 мм")st.write("• Целевой правильный Сэг (SAG): спереди ~58 мм, сзади ~60 мм. Это 30% от полного хода — идеальный баланс для туристического эндуро, обеспечивающий зацеп колес на неровностях.")
### 📸 Про фотографии регулировочных винтов:
В коде я временно поставил красивые стандартные картинки мотоциклов с фотохостинга. Когда мы развернем приложение, вы сможете сфотографировать на своем **Voge DS900X** перья вилки сверху, выносную крутилку и нижний винт амортизатора, скинуть эти фото мне, и мы заменим временные картинки на ваши реальные фотографии!

<FollowUp>
Отдыхайте в дороге! Как только будете готовы запустить весь этот проект на компьютере, просто **напишите «Дома»**, и мы начнем.
</FollowUp>
