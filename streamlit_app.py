...
if df_base is not None:
    ...
    with st.expander("🛠️ ИНСТРУКЦИЯ ДЛЯ ЗАДНЕГО АМОРТИЗАТОРА"):
        ...
        # Плоский безопасный блок шпаргалки без использования конструкции with
    exp = st.expander("🏍️ Шпаргалка: Как изменить поведение мотоцикла?")
    exp.markdown("### 🛠️ Рецепты быстрой настройки подвески")
    exp.markdown("**Жестко на кочках (бьет в руки/руль)?**")
    exp.write("• *Решение:* Открутите (против часовой стрелки в сторону **S**) винт **Сжатия** на передней вилке на 1–2 щелчка. Это сделает вилку мягче при обработке неровностей.")
    exp.markdown("**Мотоцикл раскачивается как лодка (после прыжка или волны)?**")
    exp.write("• *Решение:* Подвеске не хватает контроля возврата пружины. Закрутите (по часовой стрелке в сторону **H**) винты **Отбоя** на передней вилке и снизу заднего амортизатора на 1–2 щелчка.")
    exp.markdown("**Задняя подвеска пробивает до упора (с пассажиром или багажом)?**")
    exp.write("• *Решение:* Не хватает жесткости пружины. Накрутите черную выносную рукоятку **Преднатяга** сжатия задней пружины по часовой стрелке на 3–5 щелчков больше, чем выдает калькулятор.")
    exp.markdown("**Морда мотоцикла задирается, руль пустой и легкий на скорости?**")
    exp.write("• *Решение:* Мотоцикл слишком просел назад. Увеличьте **Преднатяг** задней пружины рукояткой на 2–3 щелчка, либо уменьшите **Преднатяг** передней вилки на 0.5–1 риску, чтобы опустить нос байка.")
    exp.markdown("**Мотоцикл неохотно заходит в повороты (сопротивляется рулению)?**")
    exp.write("• *Решение:* Перед завышен, а зад слишком занижен. Добавьте 2–3 щелчка **Преднатяга** сзади, чтобы приподнять хвост мотоцикла. Геометрия изменится, и байк станет рулиться намного острее.")
